import theme
from structures.dataclasses import Rect, SensorReading
...
class AirQualityMenu(Container):
    def __init__(self, rect: Rect) -> None:
        ...
        row_y = 140
        self.temp_icon    = Label(Rect(margin - 5, row_y, 32, 36), chr(theme.Icon.TEMPERATURE), theme.icon_style(theme.Palette.WARM))
        self.temperature  = Label(Rect(padding + 38 - 3, row_y, 110, 36), "--", theme.value_style())
        self.hum_icon     = Label(Rect(110 + margin + padding - 3, row_y, 32, 36), chr(theme.Icon.HUMIDITY), theme.icon_style(theme.Palette.COOL))
        self.humidity     = Label(Rect(110 + margin + 36 + padding - 3, row_y, 110, 36), "--", theme.value_style())
